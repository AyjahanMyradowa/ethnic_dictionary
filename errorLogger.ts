...
export const setupErrorLogging = () => {
  ...
  if (typeof window !== 'undefined') {
    ...
    // Only set up unhandled promise rejection handler on web platform
    if (Platform.OS === 'web') {
      try {
        // Remove any existing listeners first
        const existingHandler = (window as any).__unhandledRejectionHandler;
        if (existingHandler) {
          window.removeEventListener('unhandledrejection', existingHandler);
        }

        // Create new handler
        const unhandledRejectionHandler = (event: PromiseRejectionEvent) => {
          try {
            console.log('Handling unhandled promise rejection:', event);
        ...
        (window as any).__unhandledRejectionHandler = unhandledRejectionHandler;
        
        // Add the event listener
        window.addEventListener('unhandledrejection', unhandledRejectionHandler);
        console.log('Unhandled promise rejection handler set up successfully');
      } catch (e) {
        console.error('Failed to set up unhandled promise rejection handler:', e);
      }
    }
  }
